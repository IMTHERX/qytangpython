...
# print('格式化打印输出：\n\n'
#       f'{printf_info[0]:>10} : {re_asa_conn[0]:<15} |' # 输出src ：src的IP地址
#       f'{printf_info[1]:>10} : {re_asa_conn[1]:<15} |' # 输出src_p ：src的prot号码
#       f'{printf_info[2]:>10} : {re_asa_conn[2]:<15} |' # 输出dst ：dst的IP地址
#       f'{printf_info[3]:>10} : {re_asa_conn[3]:<15} |\n' # 输出dst_p ：dst的prot号码
#       f'{printf_info[4]:>10} : {re_asa_conn[4]:<15} |' # 输出bytes ：bytes的数值
#       f'{printf_info[5]:>10} : {re_asa_conn[5]:<15} |') # 输出flags ：flags的数值

# for key,value in dict_asa_conn.items():
#     print(key[0],value[0])
    # print(f'key:{x:<10}value:{y:<10}')